...
public class Portal : MonoBehaviour
{
	...
	[Tooltip("Oblique Projection clips the camera exactly to the portal rectangle. This is really good if you've got nearby objects. Unfortunately, it also screws with the skybox on Gear XR.")]
	...
	void Awake() {
#if USES_STEAM_XR
		Debug.Log("This build is set up to run with Steam XR (Vive / Rift). To enable another headset or run without XR please edit your settings in Window -> Portal State Manager.");
#elif USES_OPEN_XR
		Debug.Log("This build is set up to run with Open XR (Rift / Gear XR). To enable another headset or run without XR please edit your settings in Window -> Portal State Manager.");
#elif USES_AR_KIT
...
#else
        Debug.Log("This build is set up to run without XR or ARKit. To enable XR / AR support please edit your settings in Window -> Portal State Manager.");
#endif

#if USES_OPEN_XR
        Shader.SetGlobalInt("OpenXRRender", 1);
#else
        Shader.SetGlobalInt("OpenXRRender", 0);
#endif
    }
...
#if USES_OPEN_XR
        OXRCameraRig rig = GameObject.FindObjectOfType<OXRCameraRig>();
        Assert.IsNotNull(rig, "To use Open XR Portal mode you need to have an OXRCameraRig in your scene.");
...
#if USES_STEAM_XR || USES_OPEN_XR
		if (camera.stereoEnabled) {  // IE: If we're in XR

/* Open XR Special */
#if USES_STEAM_XR
			this.RenderSteamXR (camera);
#endif

/* Gear XR Special */
#if USES_OPEN_XR
			this.RenderOpenXR (camera);
#endif
        }
	...
	private void RenderSteamXR(Camera camera) {
#if USES_STEAM_XR
		if (camera.stereoTargetEye == StereoTargetEyeMask.Both || camera.stereoTargetEye == StereoTargetEyeMask.Left) {
			Vector3 eyePos = camera.transform.TransformPoint (SteamXR.instance.eyes [0].pos);
			Quaternion eyeRot = camera.transform.rotation * SteamXR.instance.eyes [0].rot;
			Matrix4x4 projectionMatrix = GetSteamXRProjectionMatrix (camera, Valve.XR.EXREye.Eye_Left);
		...
		if (camera.stereoTargetEye == StereoTargetEyeMask.Both || camera.stereoTargetEye == StereoTargetEyeMask.Right) {
			Vector3 eyePos = camera.transform.TransformPoint (SteamXR.instance.eyes [1].pos);
			Quaternion eyeRot = camera.transform.rotation * SteamXR.instance.eyes [1].rot;
			Matrix4x4 projectionMatrix = GetSteamXRProjectionMatrix (camera, Valve.XR.EXREye.Eye_Right);
	...
	private void RenderOpenXR(Camera camera) {
#if USES_OPEN_XR
        Transform trackingSpace = GameObject.FindObjectOfType<OXRCameraRig>().trackingSpace;
...
#if USES_STEAM_XR
	public static Matrix4x4 GetSteamXRProjectionMatrix (Camera cam, Valve.XR.EXREye eye)
	{
	Valve.XR.HmdMatrix44_t proj = SteamXR.instance.hmd.GetProjectionMatrix (eye, cam.nearClipPlane, cam.farClipPlane);
